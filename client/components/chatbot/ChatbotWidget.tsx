...
const ChatbotWidget = ({ autoOpen = false }: ChatbotWidgetProps) => {
    ...
    <div className="fixed bottom-5 right-5 z-[95]">
      {open && (
        <div className="w-[min(92vw,380px)] h-[min(76vh,620px)] bg-[#0b0b0b] border border-white/15 rounded-lg shadow-2xl flex flex-col overflow-hidden mb-3">
          ...
          <div ref={containerRef} className="flex-1 overflow-y-auto px-3 py-3 space-y-3" aria-live="polite">
            {messages.map((msg) => (
              <div
                key={msg.id}
          ...
          <div className="px-3 py-2 border-t border-white/10 bg-black/60">
            ...
            {leadOpen && (
              <div className="border border-primary/40 bg-primary/10 p-3 space-y-2 mb-2">
                ...
                />
                <label htmlFor="chatbot-lead-email" className="sr-only">Email</label>
                <input
                  ...
                  className="w-full px-2 py-2 text-sm bg-black/50 border border-white/20 outline-none"
                />
                <label htmlFor="chatbot-lead-phone" className="sr-only">Phone</label>
                <input
                  id="chatbot-lead-phone"
                  ...
                  className="w-full px-2 py-2 text-sm bg-black/50 border border-white/20 outline-none"
                />
                <label htmlFor="chatbot-lead-budget" className="sr-only">Budget</label>
                <input
                  id="chatbot-lead-budget"
                  value={leadForm.budget}
                  onChange={(e) => setLeadForm((prev) => ({ ...prev, budget: e.target.value }))}
                  placeholder="Project Budget (e.g. INR 5L-10L)"
                  className="w-full px-2 py-2 text-sm bg-black/50 border border-white/20 outline-none"
                />
                <label htmlFor="chatbot-lead-timeline" className="sr-only">Timeline</label>
                <input
                  id="chatbot-lead-timeline"
                  value={leadForm.timeline}
                  onChange={(e) => setLeadForm((prev) => ({ ...prev, timeline: e.target.value }))}
                  placeholder="Timeline (e.g. 8 weeks)"
                  className="w-full px-2 py-2 text-sm bg-black/50 border border-white/20 outline-none"
                />
                <label htmlFor="chatbot-lead-details" className="sr-only">Project details</label>
                <textarea
                  id="chatbot-lead-details"
            ...
            <div className="flex items-center gap-2">
              <label htmlFor="chatbot-input" className="sr-only">Ask a question</label>
              <input
                id="chatbot-input"
                value={input}
