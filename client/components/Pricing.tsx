...
const Pricing: React.FC = () => {
  ...
  const checkoutCloseButtonRef = useRef<HTMLButtonElement>(null);
  ...
  const openCheckout = (tier: (typeof tiers)[number], triggerEl?: HTMLElement) => {
    if (!payableTierIds.has(tier.id)) {
      handlePlanSelect(tier.name);
      return;
    }
    checkoutTriggerRef.current = triggerEl || (document.activeElement as HTMLElement | null);
  ...
  useEffect(() => {
    const isOpen = Boolean(checkoutTier);
    if (!isOpen || !checkoutDialogRef.current) {
      return;
    }
    ...
    const getFocusableElements = () =>
      Array.from(dialog.querySelectorAll<HTMLElement>(focusSelector)).filter((el) => !el.hasAttribute('aria-hidden'));

    const initialFocus = checkoutCloseButtonRef.current || getFocusableElements()[0];
    initialFocus?.focus();
  ...
  }, [checkoutTier, checkoutStatus]);

  useEffect(() => {
    const isOpen = Boolean(checkoutTier);
    if (!isOpen && wasCheckoutOpenRef.current) {
        ...
        <div className="grid lg:grid-cols-3 gap-8">
          {tiers.map((tier) => (
            ...
            >
              ...
              <button
                type="button"
                onClick={(event) => openCheckout(tier, event.currentTarget)}
          ...
          >
            <div className="flex items-start justify-between gap-6">
              <div>
                ...
                <h2 id="checkout-dialog-title" className="text-2xl font-semibold mt-2">{checkoutTier.name} Plan</h2>
                <p id="checkout-dialog-description" className="text-gray-400 mt-2 text-sm">Secure checkout powered by PayU India.</p>
              </div>
              <button
            ...
            <form className="mt-6 space-y-4" onSubmit={handleCheckoutSubmit}>
              ...
              <div>
                <label htmlFor="checkout-name" className="text-xs uppercase tracking-[0.3em] text-gray-400">Full Name</label>
                <input
                  id="checkout-name"
                  type="text"
                  name="name"
                ...
                />
                {checkoutErrors.name && <p id="checkout-name-error" className="text-xs text-red-400 mt-2">{checkoutErrors.name}</p>}
              </div>

              <div>
                <label htmlFor="checkout-email" className="text-xs uppercase tracking-[0.3em] text-gray-400">Email</label>
                <input
                  id="checkout-email"
                  type="email"
                  name="email"
                  ...
                  aria-describedby={checkoutErrors.email ? 'checkout-email-error' : undefined}
                ...
                />
                {checkoutErrors.email && <p id="checkout-email-error" className="text-xs text-red-400 mt-2">{checkoutErrors.email}</p>}
              </div>

              <div>
                <label htmlFor="checkout-phone" className="text-xs uppercase tracking-[0.3em] text-gray-400">Phone</label>
                <input
                  id="checkout-phone"
                  type="tel"
                  name="phone"
                ...
                />
                {checkoutErrors.phone && <p id="checkout-phone-error" className="text-xs text-red-400 mt-2">{checkoutErrors.phone}</p>}
              </div>
