...
export async function GET(_req: Request, context: { params: Promise<{ slug: string }> }) {
  try {
    ...
    if (!backendBaseUrl) {
      captureApiMessage('Blog backend is not configured.', { route: '/api/blog/posts/[slug]', method: 'GET', status: 500 });
    ...
    captureApiException(error, { route: '/api/blog/posts/[slug]', method: 'GET' });
