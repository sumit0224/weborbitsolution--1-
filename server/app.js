...
const seoTitleText = z
  .string()
  .transform((val) => stripTags(val))
  .refine((val) => val.length >= SEO_TITLE_MIN && val.length <= SEO_TITLE_MAX, {
    message: `Blog title must be between ${SEO_TITLE_MIN} and ${SEO_TITLE_MAX} characters.`,
  });

const seoDescriptionText = z
  .string()
  .transform((val) => stripTags(val))
  .refine((val) => val.length >= SEO_DESCRIPTION_MIN && val.length <= SEO_DESCRIPTION_MAX, {
    message: `Meta description must be between ${SEO_DESCRIPTION_MIN} and ${SEO_DESCRIPTION_MAX} characters.`,
  });
...
const blogPostSchema = z.object({
  ...
  metaTitle: seoTitleText,
  metaDescription: seoDescriptionText,
